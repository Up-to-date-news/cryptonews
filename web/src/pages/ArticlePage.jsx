...
export default function ArticlePage() {
  ...
  // Tamil is the one curated (Tier 1) translation, generated by the
  // pipeline alongside the English content — render it directly instead
  // of running it through the Google Translate widget. Falls back to the
  // English content (which the widget then machine-translates, same as
  // any other language) for articles enriched before this existed, or
  // manually-created admin posts that never get a Tamil version.
  const isCuratedTamil = language === 'ta' && Boolean(article?.content_ta);
  const displayContent = isCuratedTamil ? article.content_ta : article?.content;

  const description = displayContent
    ? displayContent.slice(0, 160)
